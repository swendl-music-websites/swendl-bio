jQuery(function($) {
    ...
    window.ScrollEffects = function() {
        ...
        if (isMobile()) {
            ...
            $('.smooth-scroll main, .has-parallax, nav, .clapat-slider-wrapper:not(.content-slider), .showcase-lists .clapat-sync-slider, .next-project-image-wrapper, .slider-fixed-content').css({
                'height': winHeight
            });
        ...
        gsap.utils.toArray('.carousel-shortcode-pin').forEach((carouselShortcodePin) => {
            ...
            function setCarouselShortcodeWrapperProperties() {
                gsap.set(carouselShortcodeThumbs, {
                    height: window.innerHeight
                });
                gsap.set(carouselShortcodeCaption, {
                    height: window.innerHeight
                });
            }
        ...
        gsap.utils.toArray('.list-rotator-wrapper').forEach((listRotatorWrapper) => {
            ...
            if (!isMobile()) {
                ...
                function setlistRotatorProperties() {
                    gsap.set(listRotatorWrapper, {
                        height: window.innerHeight * 3
                    });
                    gsap.set(listRotatorHeight, {
                        height: window.innerHeight * 4
                    });
                    gsap.set(listRotator, {
                        height: window.innerHeight
                    });
            ...
            gsap.utils.toArray('.pinned-gallery').forEach((pinnedGallery) => {
                ...
                if (pinnedGallery && pinnedGallery.classList.contains('random-img-ratation')) {
                    ...
                    gsap.utils.toArray(rotatedImages).forEach((pImage, i, arr) => {
                        ...
                        gsap.set(pImage.querySelector('img'), {
                            rotation: rotation
                        });
                    });
                }
        ...
        hasParallax.forEach(function(hParallax) {
            ...
            var parallax = gsap.fromTo([bgImage, bgVideo], {
                y: '-20%',
                scale: 1.15
            }, {
                y: '20%',
                    ...
                    $body.on('mouseup mousemove', function handler(evt) {
                        if (evt.type === 'mouseup') {
                            ...
                            $('#itemsWrapperLinks .trigger-item-link, #itemsWrapperLinks .trigger-item-link-secondary').on('click', function() {
                                ...
                                gsap.to('#hero, #show-filters, .item-caption-wrapper, .showcase-portfolio .slide-caption, #page-nav, footer, .fadeout-element', {
                                    duration: 0.5,
                                    opacity: 0,
                                    ease: Power4.easeInOut
                                });
    ...
    window.Lightbox = function() {
        ...
        function hideDetails() {
            ...
            Flip.fit(detailImage, sourceItem, {
                scale: true,
                fitChild: detailImage
            });
...
//--------------------------------------------------
//					Navigation
// Description: Custom script for navigation to scroll
//--------------------------------------------------
// Version: 1.0
//--------------------------------------------------
// Add this to your navigation script

(function($) {
    "use strict";
    
    // Enhanced content update function
    function updateContentWithBackground(newDocument) {
        // Preserve the text stroke background
        const textStrokeContainer = $('.text-stroke-container');
        const newTextStroke = $(newDocument).find('.text-stroke-container');
        
        // Update content with proper handling of the background
        return new Promise((resolve) => {
            // Fade out current content except background
            $('#content-scroll > *:not(.text-stroke-container)').fadeOut(300, function() {
                // Update the main content
                const newContent = $(newDocument).find('#content-scroll > *:not(.text-stroke-container)');
                $('#content-scroll').html(newContent);
                
                // Update text stroke content while preserving the container
                if (newTextStroke.length) {
                    textStrokeContainer.html(newTextStroke.html());
                }
                
                // Ensure background stays visible
                gsap.set('.text-stroke-container', {
                    clearProps: "all"
                });
                
                // Setup scroll-based animation for text stroke
                setupTextStrokeAnimation();
                
                // Fade in new content
                $('#content-scroll > *').fadeIn(300, resolve);
            });
        });
    }
    
    // Text stroke scroll animation setup
    function setupTextStrokeAnimation() {
        const textStroke = document.querySelector('.text-stroke');
        if (!textStroke) return;
        
        // Kill existing scroll triggers for this element
        if (textStroke._scrollTrigger) {
            textStroke._scrollTrigger.kill();
        }
    ...
    // Add styles to ensure proper layering
    const styles = `
        .text-stroke-container {
            position: fixed;
            z-index: -1;
            width: 100%;
            height: 100%;
    ...
    function navigateTo(url, pushState = true) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                // Update content with background handling
                updateContentWithBackground(newDocument).then(() => {
                    if (pushState) {
    ...
    // Export functions
    window.setupTextStrokeAnimation = setupTextStrokeAnimation;
